...
public class SessionManager
	 ...
	 */
	public static ConcurrentHashMap<BareJID, XMPPSession> sessionsByNodeId = new ConcurrentHashMap<BareJID, XMPPSession>(
			100000);
	public static ConcurrentHashMap<String,String> userPresenceMap = new ConcurrentHashMap<String, String>();
	...
	public void handleLogin(BareJID userId, XMPPResourceConnection conn) {
		...
		System.out.println("=========handleLogin========" + "===userid==" + userId + " ==conn== "+conn);
	...
	public void handleLogout(BareJID userId, XMPPResourceConnection conn) {
		...
		//System.out.println("===========" + userId + "========" + conn.toString());
	...
	public void initialize() {
		super.initialize();
		//System.out.println("==========initialize =====smResourceConnection===");
	...
	protected void closeConnection(XMPPResourceConnection connection, JID connectionId, String userId,
		...
		if (connection != null) {
			...
			synchronized (connection) {
				...
				//System.out.println("=====222222======" + connection + "========" + closeOnly);
...
//    }
	...
	protected void closeSession(XMPPResourceConnection conn, boolean closeOnly) {
		...
		try {
			System.out.println(conn.getUserName()+" ====下线了");
			getUserPresence(conn.getUserName(), "");
...
//			e.printStackTrace();
		...
		try {
			if (conn.isAuthorized()) {
				...
				System.out.println("===Closing connection for ==" + userJid);
	...
	protected XMPPResourceConnection loginUserSession(JID conn_id, String domain, BareJID user_id, String resource,
		...
		try {
			...
			//System.out.println("=====doAuth===55555====");
			conn.authorizeJID(user_id, false);
			//System.out.println("=======loginUserSession======" + user_id);
			if (conn.isAuthorized()) {
...
//			pt.addItem(sessionCloseProc, iqc, connection);
				...
				try {
					...
					if (prebind || isTrusted) {
						...
						if (available) {
							...
							if (connection != null) {
								//System.out.println("=====33333======" + connection );
	...
	protected void registerNewSession(BareJID userId, XMPPResourceConnection conn) {
		synchronized (conn) {
			...
			if (session == null) {
				...
				System.out.println("===Created new XMPPSession for ==" + userId);
			} else {
			...
			try {
					System.out.println(conn.getUserName()+" ===上线了 ");
					getUserPresence(conn.getUserName(), "online");
				} catch (NotAuthorizedException e) {
	...
	public static XMPPSession getJIDStatus(BareJID jid) {
		return sessionsByNodeId.get(jid);
	}
	...
	public static class SessionCloseProc
		...
		public void process(Packet packet, XMPPResourceConnection session, NonAuthUserRepository repo,
			...
			System.out.println("=====222222==sm.closeConnection====");
	...
	private class ConnectionCheckCommandHandler
		...
		public void responseReceived(Packet packet, Packet response) {
			if (response.getType() == StanzaType.error) {
				...
				String userJid = Command.getFieldValue(packet, "user-jid");
				System.out.println("=====111==closeConnection====");
		...
		public void timeOutExpired(Packet packet) {
			...
			String userJid = Command.getFieldValue(packet, "user-jid");
			System.out.println("=====333==closeConnection====");
			closeConnection(null, packet.getTo(), userJid, false);
		}
	}
	...
	private class ProcessorWorkerThread
		...
		public void process(QueueItem item) {
			...
			try {
				//System.out.println("=====1111111==process====" + item.getPacket().toString());
	...
	private class SessionCloseTimer
		...
		public void run() {
			...
			if (conn != null && (sessId == null || sessId.equals(conn.getSessionId()))) {
				...
				//System.out.println("=====444==closeConnection====");
	...
	public static class SMPacketWriter implements PacketWriter {
		...
		public synchronized static String getUserPresence(String id,String presence){
		if(presence != null) {
			if(presence.length() == 0) {
				userPresenceMap.remove(id);
			} else {
				userPresenceMap.put(id, presence);
			}
			return presence;
		} else {
			return userPresenceMap.get(id);
		}
	}
}
