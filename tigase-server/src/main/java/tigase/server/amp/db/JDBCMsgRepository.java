...
//			new ConcurrentSkipListMap<String, JDBCMsgRepository>();
	...
	protected static final String[] MSG_ORDER_PATH = {"message", "orderId"};
	protected static final String[] MSG_TYPEID_PATH = {"message", "typeId"};
...
//	private RestTemplateUtil restTemplateUtil;
	...
	public boolean storeMessage(JID from, JID to, Date expired, Element msg, NonAuthUserRepository userRepo)
		...
		System.out.println("==JDBCMsgRepository==storeMessage= " + msg.toString());
		try {

			long msgs_store_limit = getMsgsStoreLimit(to.getBareJID(), userRepo);
			System.out.println("==JDBCMsgRepository==storeMessage=111==" + msgs_store_limit);
			PreparedStatement insert_msg_st = data_repo.getPreparedStatement(to.getBareJID(), MSGS_ADD_MESSAGE);

			System.out.println("========JDBCMsgRepository==storeMessage===aaaaaa==" + SessionManager.sessionsByNodeId.keySet());

			System.out.println("========JDBCMsgRepository==storeMessage===ccc==" + msg.getChildCData(MSG_BODY_PATH));

			synchronized (insert_msg_st) {

				//如果接收方不在线，则推送消息
				if(!SessionManager.sessionsByNodeId.containsKey(to.getBareJID())){
					System.out.println("========JDBCMsgRepository==storeMessage===bbbbb==");

					Map map = new HashMap();
					map.put("from",from.getBareJID().toString());
					map.put("to",to.getBareJID().toString());
					map.put("body",storePlaintextBody?msg.getChildCData(MSG_BODY_PATH):null);
					map.put("orderId",storePlaintextBody?msg.getChildCData(MSG_ORDER_PATH):null);
					map.put("typeId",storePlaintextBody?msg.getChildCData(MSG_TYPEID_PATH):null);
					try {
						System.out.println("===========restTemplateUtil=====" + map );
						//120
						//String str = HttpUtil.sendPost("http://192.168.1.120:8889/marchInto/getOfflineMessage", map);
						//121
						//String str = HttpUtil.sendPost("http://192.168.1.121:8889/marchInto/getOfflineMessage", map);
						//19
						//String str = HttpUtil.sendPost("http://119.23.224.19:8889/marchInto/getOfflineMessage", map);

						String str = HttpUtil.sendPost("http://tx.antblock.io:8889/marchInto/getOfflineMessage", map);
						System.out.println("==========restTemplateUtil===== "+ str);
					}catch (Exception e){
						System.out.println("========JDBCMsgRepository==push message fail=");
					}
				}
				...
				System.out.println("==JDBCMsgRepository==storeMessage=2222==" + msg_type);
