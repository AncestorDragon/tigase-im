...
public class XMPPIOService<RefObject>
    ...
    protected void addReceivedPacket(final Packet packet) {
        if (firstPacket) {
            if ("policy-file-request" == packet.getElemName()) {
                ...
                String cross_domain_policy = (this.serviceListener instanceof ConnectionManager)
                        ...
                        : null;
                System.out.println("==addReceivedPacket==aaa==" + cross_domain_policy);
        ...
        if (packet.getElemName() == ACK_NAME) {
            ...
            System.out.println("==addReceivedPacket==bbb=" + packet.getElemName());
            if (packet.getElemChild(Message.ELEM_NAME) != null) {
                Packet bakPacket = packet;
                bakPacket.setPacketFrom(packet.getPacketFrom());
                bakPacket.setPacketTo(packet.getStanzaFrom());
                System.out.println("==addReceivedPacket==ccccccc=" + bakPacket.toString());
                addPacketToSend(bakPacket);
            }
        }
    }
    ...
    private void sendAck(Packet packet) {
        System.out.println("=====sendAck======white_char_ack===" + white_char_ack);
        System.out.println("=====sendAck======xmpp_ack===" + xmpp_ack);
        ...
        if (white_char_ack || xmpp_ack) {
            ...
            if (xmpp_ack) {
                ...
                System.out.println("=====sendAck======req===" + req);
                if (req != null) {
            ...
            System.out.println("=====sendAck======ack===" + ack);
            if (ack != null) {
                try {
                    writeRawData(ack);
    ...
    protected void moveParsedPacketsToReceived(boolean sendAck) {
        ...
        while ((elem = elems.poll()) != null) {
            try {
                ...
                System.out.println("====moveParsedPacketsToReceived====" + elem.toString());
                ...
                if (sendAck) {
                    System.out.println("====moveParsedPacketsToReceived==sendAck==" + pack.toString());
                    sendAck(pack);
                }
