...
public class ScramCallbackHandler
	...
	public void handle(Callback[] callbacks) throws IOException, UnsupportedCallbackException {
		for (int i = 0; i < callbacks.length; i++) {
			...
			System.out.println("=====handle====callbacks.length===" + callbacks[i].getClass().getSimpleName());
	...
	protected void handleAuthorizeCallback(AuthorizeCallback authCallback) {
		...
		System.out.println("======handleAuthorizeCallback=====111===authCallback==" + authCallback);
		String authenId = authCallback.getAuthenticationID();
		System.out.println("======handleAuthorizeCallback====222==authenId====" + authenId);
		if (log.isLoggable(Level.FINEST)) {
			log.log(Level.FINEST, "AuthorizeCallback: authenId: {0}", authenId);
		}

		fetchCredentials();
		System.out.println("======handleAuthorizeCallback====333==accountDisabled====" + accountDisabled);
		if (accountDisabled) {
	...
	protected void handleCallback(Callback callback) throws UnsupportedCallbackException, IOException {
		System.out.println("===AAAAA====handleCallback=========" + callback);
	...
	protected void handleSaltedPasswordCallbackCallback(SaltedPasswordCallback callback) {
		...
		System.out.println("=========handleSaltedPasswordCallbackCallback====111====");
		fetchCredentials();
		System.out.println("=========handleSaltedPasswordCallbackCallback===222=====" + credentialsEntry );
		if (credentialsEntry != null) {
			System.out.println("=========handleSaltedPasswordCallbackCallback===3333====" + Arrays.toString(credentialsEntry.getSaltedPassword()));
	...
	private void fetchCredentials() {
		...
		System.out.println("=====fetchCredentials===1111===" + credentialsFetched);
		...
		try {
			...
			System.out.println("=====fetchCredentials===jid===" + jid + "=======username===" + username + "==repo==" + repo);
			Credentials credentials = repo.getCredentials(jid, username);
			System.out.println("=====fetchCredentials===credentials===" + credentials);
			if (credentials == null) {
