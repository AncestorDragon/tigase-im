...
public abstract class AbstractSaslSCRAM
	...
	public byte[] evaluateResponse(byte[] response) throws SaslException {
		try {
			System.out.println("=========evaluateResponse====step======" + step + "=====" + Arrays.toString(response));
			switch (step) {
	...
	protected byte[] processClientFirstMessage(byte[] data)
		...
		System.out.println("==processClientFirstMessage==pc ====" +pc);
		handleCallbacks(nc, ai, ic, sc, pc, cc);

		if (pc.getSaltedPassword() == null) {
			throw new SaslException("Unknown user");
		}
		System.out.println("==processClientFirstMessage==cfmUsername=" + cfmUsername );


		System.out.println("==processClientFirstMessage==pwd=" + Arrays.toString(pc.getSaltedPassword()) );
	...
	protected byte[] processClientLastMessage(byte[] data)
		...
		//System.out.println("======processClientLastMessage===1111====" + r.matches());
		if (!r.matches()) {
			throw new SaslException("Bad challenge syntax");
		}
		...
		if (!clmNonce.equals(sfmNonce)) {
			//System.out.println("======processClientLastMessage===444====");
		...
		//System.out.println("======processClientLastMessage==bbb=555===="+ authMessage + "===" + Arrays.toString(clientSignature) + "====" +  Arrays.toString(clientKey));
		byte[] clientProof = xor(clientKey, clientSignature);

		byte[] dcp = Base64.decode(clmProof);
		//System.out.println("======processClientLastMessage==aaaa=555====" + Arrays.toString(clientProof) + "=====" + Arrays.toString(dcp));
		boolean proofMatch = Arrays.equals(clientProof, dcp);

		if (proofMatch == false) {
			//System.out.println("======processClientLastMessage===555====");
		...
		if (ac.isAuthorized() == true) {
			...
			//System.out.println("======processClientLastMessage===666====");
