...
public class TigaseCustomAuth
	...
	public Credentials getCredentials(BareJID user, String username) throws TigaseDBException {
		...
		System.out.println("===TigaseCustomAuth====getCredentials======" + userlogin_active );
		if (userlogin_active) {
			...
			return new SingleCredential(user, getAccountStatus(user), new Credentials.Entry() {
				...
				public boolean verifyPlainPassword(String password) {
					try {
						System.out.println("======TigaseCustomAuth====verifyPlainPassword==user==" + user + "==password=" + password );
		...
		try {
			System.out.println("======TigaseCustomAuth=========11=user==" + user);
			...
			synchronized (get_credentials) {
				...
				try {
					...
					System.out.println("======TigaseCustomAuth=====rs====" + rs);

					while (rs.next()) {
						String mechanism = rs.getString(1);
						String value = rs.getString(2);
						accountStatus = AccountStatus.byValue(rs.getInt(3));
						System.out.println("======TigaseCustomAuth=====mechanism====" + mechanism + "==value==" + value + "==entries==" + entries);
						entries.add(new DefaultCredentials.RawEntry(mechanism, value));
					}

				} finally {
					System.out.println("======TigaseCustomAuth== data_repo ==="+ data_repo);
					data_repo.release(null, rs);
				}
			}
			System.out.println("======TigaseCustomAuth== DefaultCredentials ==="+ entries);
	...
	private boolean saslPlainAuth(final Map<String, Object> props)
		...
		System.out.println("===saslPlainAuth======");
				...
				if (callbacks[i] instanceof RealmCallback) {
					...
					if (callbacks[i] instanceof NameCallback) {
						...
						if (callbacks[i] instanceof PasswordCallback) {
							...
							try {
								System.out.println("====handle===huang==aaaa===");
