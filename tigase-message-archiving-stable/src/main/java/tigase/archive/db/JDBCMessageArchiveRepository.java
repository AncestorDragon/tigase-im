...
public class JDBCMessageArchiveRepository<Q extends QueryCriteria>
	...
	protected Long archiveMessage(BareJID owner, JID buddy, Direction direction, Date timestamp, Element msg,
		...
		try {
			...
			String orderId = storePlaintextBody ? msg.getChildCData(MSG_ORDER_PATH) : null;
			String typeId = storePlaintextBody ? msg.getChildCData(MSG_TYPE_PATH) : null;
			...
			synchronized (add_message_st) {
				try {
					...
					add_message_st.setString(i++, orderId);
					if(null != typeId) {
						add_message_st.setInt(i++, Integer.parseInt(typeId));
					}else{
						add_message_st.setInt(i++, 1);
					}
